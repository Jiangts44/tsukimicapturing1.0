(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined'
    ...
    : ((global = typeof globalThis !== 'undefined' ? globalThis : global || self),
      factory((global.modernScreenshot = {})));
...
`);
  ...
  function copyCssStyles(node, cloned, isRoot, context) {
    ...
    if (IN_CHROME) {
      ...
      if (
        (style.get('overflow-x')?.[0] === 'hidden' || style.get('overflow-y')?.[0] === 'hidden') &&
  ...
  async function cloneNode(node, context, isRoot = false, addWordToFontFamilies) {
    ...
    if (ownerDocument && ownerWindow && isElementNode(node) && (isHTMLElementNode(node) || isSVGElementNode(node))) {
      ...
      const addWordToFontFamilies2 = families
        ...
        : void 0;
      await copyPseudoClass(node, cloned2, copyScrollbar, context, addWordToFontFamilies2);
      ...
      cssRules
        .filter(
          cssRule =>
            ...
            splitFontFamily(cssRule.style.getPropertyValue('font-family'))?.some(val => fontFamilies.has(val)),
        )
        .forEach(value => {
          const rule = value;
